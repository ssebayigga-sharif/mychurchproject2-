...
export const getThreads = async (): Promise<MessageThread[]> => {
  const res = await axios.get<Record<string, Omit<MessageThread, "id">> | null>(
    `${BASE}/threads.json`
    );
    const data = res.data
  if (!data) return [];
...
export const getMessages = async (threadId: string): Promise<InternalMessage[]> => {
  const res = await axios.get<Record<string, Omit<InternalMessage, "id">> | null>(
    `${BASE}/threads/${threadId}/messages.json`
    );
    const data = res.data;
  if (!data) return [];
  return Object.entries(data)
...
export const createThread = async (
  ...
  const res = await axios.post<{ name: string }>(`${BASE}/threads.json`, {
    subject,
    participants,
    createdAt: now,
    lastMessageAt: now,
  ...
  return res.data.name; // Firebase returns the generated key as `name`
};
