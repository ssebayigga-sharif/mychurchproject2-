        ...
        <Column lg={16} md={8} sm={4}>
          <div className={styles.pageHeader}>
            <div>
              ...
              <p className={styles.subtitle}>Record and monitor member presence for church events</p>
            </div>
            {toast && (
            ...
            >
              ...
              {programs.map((p) => (
                <SelectItem
                  key={p.id}
                  value={p.id}
                  text={`${p.title} — ${new Date(p.date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}`}
                />
              ))}
            </Select>
          </div>
          ...
          {isLoading ? (
            ...
            <div className={styles.emptyState}>
              ...
              <p className={styles.emptyText}>Choose a program from the list above to begin recording attendance.</p>
            </div>
          ) : members.length === 0 ? (
              ...
              <div className={styles.tableWrapper}>
                <DataTable rows={rows} headers={headers}>
                  {({ rows, headers, getHeaderProps, getTableProps, getTableContainerProps }) => (
                    <TableContainer {...getTableContainerProps()} title="Member Registry">
                      <Table {...getTableProps()}>
                        <TableHead>
                          <TableRow>
                            {headers.map((header) => (
                            ...
                            <TableRow key={row.id}>
                              {row.cells.map((cell) => {
                                if (cell.info.header === "avatar") {
                                  return (
                                    <TableCell key={cell.id} className={styles.avatarCell}>
                                      <div className={`${styles.avatar} ${getAvatarColor(cell.value)}`}>
                                ...
                                if (cell.info.header === "present") {
                                  const mId = row.id;
                                  return (
                                    <TableCell key={cell.id} className={styles.checkCell}>
                                      <Checkbox
                                        id={`check-${mId}`}
                                        labelText=""
                                        checked={cell.value}
